...
Select concat( FIRST_NAME,' ', LAST_NAME) Name,JOB_ID ,departments.DEPARTMENT_ID from employees,departments,locations
where employees.DEPARTMENT_ID=any(Select departments.DEPARTMENT_ID  from departments, locations 
where departments.LOCATION_ID=any(Select LOCATION_ID from locations where CITY='London'))
group by Name ;
...
Select A.EMPLOYEE_ID, A.LAST_NAME , A.MANAGER_ID, B.LAST_NAME  from employees A, employees B
where A.MANAGER_ID=B.EMPLOYEE_ID ;

Select A.EMPLOYEE_ID, A.LAST_NAME , A.MANAGER_ID, B.LAST_NAME  from employees A left outer join employees B
...
where HIRE_DATE>ANY(Select HIRE_DATE from employees where concat( FIRST_NAME,' ', LAST_NAME)  like '%Jones%');

-- Write a query to get the department name and number of employees in the department.
select DEPARTMENT_NAME ,count(EMPLOYEE_ID) NO_OF_EMPLOYEES from employees, departments
where employees.DEPARTMENT_ID =departments.DEPARTMENT_ID group by DEPARTMENT_NAME ;
...
Select employees.EMPLOYEE_ID,jobs.JOB_TITLE,datediff(END_DATE,HIRE_DATE) days from employees,jobs,job_history
where employees.JOB_ID=jobs.JOB_ID And employees.DEPARTMENT_ID=job_history.DEPARTMENT_ID And employees.DEPARTMENT_ID=90
group by employees.EMPLOYEE_ID 
order by employees.EMPLOYEE_ID asc;

-- Write a query to display the department ID and name and first name of manager. 
select A.DEPARTMENT_ID,concat( A.FIRST_NAME,' ', A.LAST_NAME) Name, B.FIRST_NAME from employees A left outer join employees B
on B.EMPLOYEE_ID=A.MANAGER_ID;

-- 	Write a query to display the department name, manager name, and city.
select DEPARTMENT_NAME,concat( employees.FIRST_NAME,' ', employees.LAST_NAME) MGRName ,CITY FROM employees, departments, locations
where employees.EMPLOYEE_ID=departments.MANAGER_ID AND locations.LOCATION_ID=departments.LOCATION_ID
GROUP BY DEPARTMENT_NAME;

-- Write a query to display the job title and average salary of employees.
Select JOB_TITLE,avg(SALARY) avg_sal from employees,jobs
...
Select JOB_TITLE, concat( FIRST_NAME,' ', LAST_NAME) Name, SALARY-MIN_SALARY SAL_DIFF from employees,jobs
where employees.JOB_ID=jobs.JOB_ID
group by JOB_TITLE;
...
where job_history.EMPLOYEE_ID=employees.EMPLOYEE_ID And employees.SALARY>10000;
...
Select employees.EMPLOYEE_ID,year(job_history.END_DATE)-year(HIRE_DATE) from employees,job_history 
where employees.EMPLOYEE_ID=job_history.EMPLOYEE_ID group by employees.EMPLOYEE_ID;
...
select  DEPARTMENT_NAME ,departments.location_id, street_address, city, state_province, country_name from locations,countries,departments
where locations.COUNTRY_ID=countries.COUNTRY_ID And departments.LOCATION_ID=locations.LOCATION_ID ;
